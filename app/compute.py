...
def get_live_ferries(now=None, schedule=None, boarding_window_min=120):
    """Checks the REAL schedule against the current time-of-day and returns
    sailings that are either genuinely at sea right now (real elapsed-time
    progress along the route), or docked and boarding — real ferries
    genuinely sit at port before departure, so a sailing within
    `boarding_window_min` minutes of its real departure time is shown as
    boarding at the origin port. Not invented — just extending what we
    already honestly know about the real schedule."""
    if now is None:
    ...
    for _, row in schedule.iterrows():
        ...
        has_arrival = "arrival_time" in row and pd.notna(row.get("arrival_time"))
        arr_total = None
        if has_arrival:
            arr_h, arr_m = parse_hour_minute(row["arrival_time"])
            arr_total = arr_h * 60 + arr_m
        ...
        for now_t in now_candidates:
            # Genuinely at sea — real elapsed-time progress along the route
            if has_arrival and dep_total <= now_t <= arr_total:
                active.append({
                    **base_info,
                    "status": "at_sea",
                    "progress": round((now_t - dep_total) / max(1, (arr_total - dep_total)), 4),
                })
                matched = True
                break
            # Docked and boarding — real departure is coming up soon, ship
            # is realistically already at the origin port.
            if dep_total - boarding_window_min <= now_t < dep_total:
                active.append({
                    **base_info,
                    "status": "boarding",
                    "progress": 0.0,
                    "minutes_until_departure": round(dep_total - now_t),
                })
                matched = True
                break
        if matched:
